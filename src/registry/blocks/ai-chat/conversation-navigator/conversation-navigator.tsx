...
export type CreateConversationNavigatorItemsOptions<TMessage extends ConversationMessageLike> = {
...
export type ConversationNavigatorProps = Omit<React.ComponentProps<'nav'>, 'onChange'> & {
...
export function createConversationNavigatorItems<TMessage extends ConversationMessageLike>(
...
export function ConversationNavigator({
  ...
  const validInternalActiveId = items.some((item) => item.id === internalActiveId && !item.disabled)
    ? internalActiveId
    : fallbackActiveId
  ...
  const navigateToItem = React.useCallback(
    ...
    [commitActiveId, onNavigate, resolveTarget, scrollBehavior, scrollBlock, shouldReduceMotion],
  )
