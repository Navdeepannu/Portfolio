...
export type WorkflowStage = 'receipt' | 'policy' | 'category' | 'approval'
...
export function ReceiptPanelContent() {
  return (
    <PanelInset>
...
export function MatchingPanelContent({
  isActive,
  shouldReduceMotion,
}: {
  isActive: boolean
...
export function ApprovalPanelContent() {
  return (
    <PanelInset>
...
export function StandalonePanelFrame({
  children,
  className,
  title,
  stage,
  ...
  return (
    <div
      role="img"
      aria-label={title}
      className={cn('relative h-52 w-64 text-left sm:h-58 sm:w-72', className)}
    >
      <WorkflowPanel
        stage={stage}
        title={title}
